    ...
    private func getRequestToken() {
        ...
        // GUARD: Did we get a successful 2XX response?
            guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode
                where statusCode >= 200 && statusCode <= 299 else {
                    displayError("There was an error with your status code it was not a 2XX number")
                    return
            }
            
        // GUARD: Was there any data returned? 
            guard let data = data else {
                displayError("There was no data")
                return
            }
        ...
        // parse the data
            ...
            do {
                ...
                displayError("Could not parse as JSON data: \(data)")
                return
            }
            ...
            guard let requestToken = parsedResult[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError("Cannot find key in \(Constants.TMDBResponseKeys.RequestToken) in \(parsedResult)")
                return
            }
    ...
    private func loginWithToken(requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            func displayError(error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Login failed (validate login)"
                }
            }
            /* 5. Parse the data */
            // GUARD: Was there an error?
    ...
    private func getSessionID(requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            func displayError(error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Failed: Getting session ID"
                }
            }
            /* 5. Parse the data */
            // GUARD: Was there an error?
            ...
            guard let sessionID = parsedResult[Constants.TMDBResponseKeys.SessionID] as? String else {
                displayError("Cannot find key in \(Constants.TMDBResponseKeys.SessionID) in \(parsedResult)")
                return
            }
    ...
    private func getUserID(sessionID: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            func displayError(error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Login failed: getting userID"
                }
            }
        ...
        /* 6. Use the data! */
            // GUARD: Is the userID in parsedResult?
            guard let userID = parsedResult[Constants.TMDBResponseKeys.UserID] as? Int else {
                displayError("Cannot find key in \(Constants.TMDBResponseKeys.UserID) in \(parsedResult)")
                return
            }
